...
export function createIndicatorOverlay(container: HTMLElement, chart: any, onOverlayChange?: () => void): () => void {
  ...
  const renderOverlay = () => {
    ...
    panels.forEach((panelId: string, index: number) => {
      ...
      actionWrap.style.cssText = `display:none;align-items:center;gap:4px;height:${titleHeaderBoxHeight}px;overflow:hidden;`;
      ...
      actionWrapRight.style.cssText = 'display:flex;align-items:center;gap:4px;opacity:0;max-width:0;overflow:hidden;transform:translateX(6px);transition:opacity 0.12s ease,max-width 0.16s ease,transform 0.12s ease;';
      ...
      const upBtn = panelIconBtn(iconSvg('up'), '위로 이동', () => {
        chart.shiftPanelOrder(panelId, -1);
        refreshAll();
      });
      upBtn.disabled = index === 0;
      upBtn.style.opacity = upBtn.disabled ? '0.38' : '1';
      if (upBtn.disabled) upBtn.style.pointerEvents = 'none';

      const downBtn = panelIconBtn(iconSvg('down'), '아래로 이동', () => {
        chart.shiftPanelOrder(panelId, 1);
        refreshAll();
      });
      downBtn.disabled = index === panels.length - 1;
      downBtn.style.opacity = downBtn.disabled ? '0.38' : '1';
      if (downBtn.disabled) downBtn.style.pointerEvents = 'none';
      ...
      actionWrapRight.appendChild(upBtn);
      actionWrapRight.appendChild(downBtn);
