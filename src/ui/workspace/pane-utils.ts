export type MainIndicatorsLike = {
  indicators: {
    ...
    kalmanAdjustedAtr?: { show: boolean };
...
export function collectMainIndicatorNames(config: MainIndicatorsLike): string[] {
  ...
  if (config.indicators.donchianChannel?.show) names.push('DC');
  ...
  if (config.indicators.autoTrendlineChannel?.show) names.push('ATL');
  ...
  if (config.indicators.kalmanAdjustedAtr?.show) names.push('KATR');
