export const INDICATOR_CATALOG = [
  { id: 'rsi', label: 'RSI', desc: 'Relative Strength Index', panel: 'sub' },
  { id: 'mfi', label: 'MFI', desc: 'Money Flow Index', panel: 'sub' },
  { id: 'momentum', label: 'Momentum', desc: 'Price momentum (close - close n bars ago)', panel: 'sub' },
  { id: 'macd', label: 'MACD', desc: 'Moving Avg Convergence Div', panel: 'sub' },
  { id: 'dmi', label: 'DMI / ADX', desc: 'Directional Movement Index', panel: 'sub' },
  { id: 'stochF', label: 'Stochastic Fast', desc: 'Fast Stochastic Oscillator', panel: 'sub' },
  { id: 'stochS', label: 'Stochastic Slow', desc: 'Slow Stochastic Oscillator', panel: 'sub' },
  { id: 'cci', label: 'CCI', desc: 'Commodity Channel Index', panel: 'sub' },
  { id: 'obv', label: 'OBV', desc: 'On Balance Volume', panel: 'sub' },
  { id: 'cvd', label: 'CVD', desc: 'Cumulative Volume Delta', panel: 'sub' },
  { id: 'atr', label: 'ATR', desc: 'Average True Range', panel: 'sub' },
  { id: 'volume', label: 'Volume', desc: 'Trading Volume', panel: 'sub' },
  { id: 'bb', label: 'Bollinger Band', desc: 'Bollinger Bands (20,2)', panel: 'main' },
  { id: 'ma', label: 'MA', desc: 'Moving Average lines (MA1, MA2, ...)', panel: 'main' },
  { id: 'ema', label: 'EMA', desc: 'Exponential Moving Average lines (EMA1, EMA2, ...)', panel: 'main' },
  { id: 'hma', label: 'HMA', desc: 'Hull Moving Average', panel: 'main' },
  { id: 'vwap', label: 'VWAP', desc: 'Volume Weighted Avg Price', panel: 'main' },
  { id: 'donchianChannel', label: 'Donchian Channel', desc: 'Highest high and lowest low price channel', panel: 'main' },
  { id: 'williamsFractal', label: 'Williams Fractal', desc: 'Variable swing high/low markers', panel: 'main' },
  { id: 'williamsAlligator', label: 'Williams Alligator', desc: 'Jaw, Teeth, and Lips SMMA trend lines', panel: 'main' },
  { id: 'parabolicSar', label: 'Parabolic SAR', desc: 'Stop and reverse trend markers', panel: 'main' },
  { id: 'smartMoneyConcepts', label: 'Smart Money Concepts', desc: 'SMC structure, BOS/CHoCH, OB, EQH/EQL, FVG', panel: 'main' },
  { id: 'supertrend', label: 'Supertrend', desc: 'ATR trend indicator', panel: 'main' },
  { id: 'autoTrendlineChannel', label: 'Auto Trendline Channel', desc: 'Linear regression trend channel with standard deviation bands', panel: 'main' },
  { id: 'statisticalTrailingStop', label: 'Statistical Trailing Stop', desc: 'Log-normal volatility trailing stop', panel: 'main' },
  { id: 'atrTrailingEmaSignal', label: 'ATR Trailing EMA Signal', desc: 'ATR trailing stop signals with optional Trend EMA filter', panel: 'main' },
  { id: 'atrTrailingStopOrigin', label: 'ATR Trailing Stop-origin', desc: 'Original ATR trailing stop signal with close-based Trend EMA', panel: 'main' },
  { id: 'bbMtfKalmanSignal', label: 'BB MTF Kalman Signal', desc: 'Bollinger Bands MTF with Kalman reversal signals', panel: 'main' },
  { id: 'kalmanAdjustedAtr', label: 'Kalman Adjusted ATR', desc: 'Kalman price baseline clamped by ATR rails', panel: 'main' },
  { id: 'zeroLagMaTrendLevels', label: 'Zero-Lag MA Trend Levels', desc: 'Zero-lag EMA trend and ATR levels', panel: 'main' },
  { id: 'footprint', label: 'Footprint', desc: 'Bid/Ask volume footprint by price level', panel: 'main' },
  { id: 'volumeProfile', label: 'Volume Profile', desc: 'Visible Range Volume Profile', panel: 'main' },
  { id: 'fixedRangeVolumeProfile', label: 'Fixed Range Volume Profile', desc: 'User-selected range volume profile', panel: 'main' },
  { id: 'vpvr', label: 'Volume Profile Visible Range', desc: 'VPVR (alias of Visible Range Volume Profile)', panel: 'main' },
  { id: 'ichimoku', label: 'Ichimoku Cloud', desc: 'Ichimoku Kinko Hyo', panel: 'main' },
  { id: 'envelope', label: 'Envelope', desc: 'Price Envelope (MA ± %)', panel: 'main' },
] as const;
