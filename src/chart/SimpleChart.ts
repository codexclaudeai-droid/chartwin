...
import { formatKUnit, formatThousandAdaptive } from '../utils/format';
...
export class SimpleChart {
  ...
  public draw() {
    ...
    const mainAxisTextAlign: CanvasTextAlign = geometry.side === 'left' ? 'right' : 'left';
    ctx.font = `600 13px ${CHART_FONT_STACK}`; ctx.textAlign = mainAxisTextAlign;
    ...
    for (let i = 0; i < tickCount; i += 1) {
      ...
      const axisTextX = geometry.side === 'left' ? (geometry.axisPad - 4) : (chartRight + 4);
    ...
    if (hoverSignal) {
      ...
      if (mainScale) {
        ...
        if (entryY >= R.top && entryY <= mainH) {
          ...
          ctx.fillStyle = lineColor;
          const entryBoxW = Math.max(20, geometry.axisPad - 4);
          const entryBoxX = geometry.side === 'left' ? 2 : (width - entryBoxW - 2);
  ...
  private handleMouseMove(e: MouseEvent) {
    ...
    if (this.drawingMoveState && this.selectedDrawingId) {
      ...
      const idx = this.drawings.findIndex((shape) => shape.id === this.selectedDrawingId);
      if (idx >= 0) {
  ...
  private handleTouchMove(e: TouchEvent) {
    ...
    if (e.touches.length === 1) {
      ...
      if (this.drawingMoveState && this.selectedDrawingId) {
        ...
        const moved = this.moveShapeByDelta(this.drawingMoveState.baseShape, dx, dy, this.selectedDrawingPart);
        const idx = this.drawings.findIndex((s) => s.id === this.selectedDrawingId);
        if (idx >= 0) {
          this.drawings[idx] = moved;
        }
        this.requestOverlayDraw();
