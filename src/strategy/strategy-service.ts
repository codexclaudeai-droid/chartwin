...
function defaultStrategies(): StrategyDefinition[] {
  ...
  const pineSource = `BUY = ta.crossover(ta.sma(close, 5), ta.sma(close, 20))
SELL = ta.crossunder(ta.sma(close, 5), ta.sma(close, 20))`;
  const pineBollingerDirected = `//@version=6
...
//plot(strategy.equity, title="equity", color=color.red, linewidth=2, style=plot.style_areabr)`;
  ...
  return [
    ...
    buildStrategyDefinition({
      ...
      language: 'pine',
      sourceCode: pineSource,
      active: true,
      version: 2,
    }),
    buildStrategyDefinition({
      id: 'strategy_pine_bbands_directed',
      name: 'Bollinger Bands Directed (Pine)',
      description: 'Bollinger breakout + trend-aware short TP / long reversal',
      language: 'pine',
      sourceCode: pineBollingerDirected,
      active: true,
      version: 7,
    }),
    buildStrategyDefinition({
    ...
    buildStrategyDefinition({
      ...
      language: 'javascript',
      sourceCode: shayHuangVwapFastSource,
      active: true,
      version: 1,
    }),
  ];
