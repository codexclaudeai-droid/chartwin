...
test('member support panel supports notification deep links to a thread', () => {
  ...
  assert.doesNotMatch(panelSource, /현재 필터: \{activeFilter\.label\}/);
  assert.doesNotMatch(panelSource, /getSupportThreadFilterPreset/);
  assert.match(panelSource, /paginatedThreads\.map/);
  assert.match(panelSource, /선택한 조건에 해당하는 문의가 없습니다/);
