...
test('session navigation source renders avatar name and role badge together', () => {
  ...
  assert.match(source, /import \{ LogIn, LogOut, UserPlus \} from 'lucide-react'/);
  assert.match(source, /<LogIn aria-hidden="true" \/>/);
  assert.match(source, /<UserPlus aria-hidden="true" \/>/);
  assert.match(source, /className="mobile-nav-link-icon session-nav-link-icon"/);
  assert.match(source, /className="mobile-nav-link-label session-nav-link-label"/);
  assert.match(source, /<LogOut aria-hidden="true" \/>/);
  assert.match(source, /className="session-logout-label"/);
  ...
  assert.match(cssSource, /\.session-logout-label\s*\{[\s\S]*?display: none/);
  assert.match(cssSource, /\.mobile-nav-panel \.session \.session-logout-label\s*\{[\s\S]*?display: inline/);
