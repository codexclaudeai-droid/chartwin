...
When modifying chart code, follow `docs/02-design/chart-modularization-rules.md`.

Key defaults:
- Keep `src/chart/SimpleChart.ts` as a coordinator, not a dumping ground.
- Put calculations in focused helper modules.
- Put canvas drawing in `src/chart/renderers` or `src/chart/drawings/*-renderer.ts`.
- Put drawing creation in `src/chart/drawings/factories`.
- Put mouse/touch/wheel/cursor decisions in `src/chart/interaction`.
- Prefer extending existing module categories before creating a new structure.
- Run `npm.cmd run build` and `node --test tests\*.test.mjs` after modularization changes.
